...
struct route {
	char route_size;
	char pointer;
	unsigned long routes[MAX_ROUTE];
};
...
#ifdef WIN32
	// Zero sized array warning. Code works fine, but is unsafe with copy and assign.
#pragma warning (push)
...
#pragma warning (pop)
private:
	// Make sure the bad copy and assign functions are inaccessible
	options( options const & );            // undefined
	options& operator=( options const & ); // undefined
#endif
};
	...
	ip_options_compile(unsigned char *iph)
{
	...
	for (l = opt->optlen; l > 0;) {
		...
		case IPOPT_LSRR:
			...
			opt->srr = (unsigned char)(optptr - iph);
			break;
		case IPOPT_RR:
			if (opt->rr) {
				pp_ptr = optptr;
			...
			opt->rr = (unsigned char)(optptr - iph);
			break;
		case IPOPT_TIMESTAMP:
			...
			if (optptr[2] <= optlen) {
				...
				case IPOPT_TS_TSONLY:
					opt->ts = (unsigned char)(optptr - iph);
					if (skb)
